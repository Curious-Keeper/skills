...
This directory is the maintained source for verifying the user-facing behavior of Notes. Read this index before driving the app, then use the matching feature file as the recipe.
...
- `Sub-features` lists short IDs with one line for each behavior.
...
- `Gotchas` lists traps that can waste or invalidate a verification run.
...
- [Search notes](search-notes.md) covers toolbar, keyboard, and CLI search with matching, empty, and clear states.
