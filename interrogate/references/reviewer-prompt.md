...
These are the project's documented standards, its invariants, and its evolution traps. An invariant has already caused a real bug here. A trap is something that used to be true and no longer is. You cannot see either from the diff, so treat them as given rather than re-deriving or questioning them. If this section is empty, the project documents none, and you review without them.
